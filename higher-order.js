...
let evenNumbers = mixedNumbers.filter(function(ele, index, arr){
  return ele % 2 === 0;
})
...
//Code Here
let postTaxPrices = prices.map(function(ele, index, arr){
  return ele * 1.07;
})
...
let totalPopulation = populations.reduce(function(total, ele, index, arr){
return total + ele;
})
...
//Code Here
let myStrongest = monstersInYourPocket.filter(function(ele){
  return ele.CP > 200;
})
...
let orderTotals = orders.map(function(ele, index, arr){
  return ele.price * (ele.tax + 1);
})
...
let bobsTotal = purchases.reduce(function(total, ele, index, arr){
  if(ele.owner === 'Bob'){
